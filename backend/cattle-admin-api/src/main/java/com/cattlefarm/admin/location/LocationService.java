...
public class LocationService {
 ...
 @Transactional public boolean deleteBarn(long id,String key){long farm=auth.currentFarmId(),user=StpUtil.getLoginIdAsLong();String path="/api/v1/barns/"+id;Long old=replay(farm,user,key,path);if(old!=null)return true;BarnResponse barn=findBarn(id);long references=count("SELECT COUNT(*) FROM cattle WHERE farm_id=? AND barn_id=?",farm,id)+count("SELECT COUNT(*) FROM herd WHERE farm_id=? AND barn_id=?",farm,id)+count("SELECT COUNT(*) FROM transfer_record WHERE farm_id=? AND (from_barn_id=? OR to_barn_id=?)",farm,id,id)+count("SELECT COUNT(*) FROM task WHERE farm_id=? AND related_barn_id=?",farm,id);if(references>0)throw new DataConflictException("该栏舍已有牛只、牛群或业务记录，不能删除，请改为停用");idem(farm,user,key,path,id);int deleted=jdbc.update("DELETE FROM barn WHERE farm_id=? AND barn_id=? AND version=?",farm,id,barn.version());if(deleted==0)throw new DataConflictException("栏舍已被其他人修改，请刷新后重试");audit(farm,user,"BARN_DELETED","BARN",id);return true;}
 @Transactional public boolean deleteHerd(long id,String key){long farm=auth.currentFarmId(),user=StpUtil.getLoginIdAsLong();String path="/api/v1/herds/"+id;Long old=replay(farm,user,key,path);if(old!=null)return true;HerdResponse herd=findHerd(id);long references=count("SELECT COUNT(*) FROM cattle WHERE farm_id=? AND herd_id=?",farm,id)+count("SELECT COUNT(*) FROM transfer_record WHERE farm_id=? AND (from_herd_id=? OR to_herd_id=?)",farm,id,id)+count("SELECT COUNT(*) FROM task WHERE farm_id=? AND related_herd_id=?",farm,id)+count("SELECT COUNT(*) FROM mixing_order WHERE farm_id=? AND target_herd_id=?",farm,id)+count("SELECT COUNT(*) FROM ration_formula WHERE farm_id=? AND target_type='HERD' AND target_object_id=?",farm,id);if(references>0)throw new DataConflictException("该牛群已有牛只、配方或业务记录，不能删除，请改为停用");idem(farm,user,key,path,id);int deleted=jdbc.update("DELETE FROM herd WHERE farm_id=? AND herd_id=? AND version=?",farm,id,herd.version());if(deleted==0)throw new DataConflictException("牛群已被其他人修改，请刷新后重试");audit(farm,user,"HERD_DELETED","HERD",id);return true;}
 ...
 private long count(String sql,Object...args){Number value=jdbc.queryForObject(sql,Number.class,args);return value==null?0:value.longValue();}
